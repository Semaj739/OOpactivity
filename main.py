...
keal = Hero("Keal", "Warrior", 7, 150)
mira = Hero("Mira", "Healer", 4, 90)
dax = Hero("Dax", "Rouge", 6, 100)

print(f"{aria.name} ({aria.role}) - Lv.{aria.level} HP:{aria.hp} XP:{aria.xp}")
print(f"{keal.name} ({keal.role}) - Lv.{keal.level} HP:{keal.hp} XP:{keal.xp}")
...
Thunder_Bow = weapon("Thunder Bow", 50, "Ranged")

print(f"{Flame_Sword.name} ({Flame_Sword.weapon_type}) - damage: {Flame_Sword.damage} enchanted: {is_enchanted}")
print(f"{Frost_Staff.name} ({Frost_Staff.weapon_type}) - damage: {Frost_Staff.damage} enchanted: {is_enchanted}")
print(f"{Shadow_Dagger.name} ({Shadow_Dagger.weapon_type}) - damage: {Shadow_Dagger.damage} enchanted: {is_enchanted}")
print(f"{Thunder_Bow.name} ({Thunder_Bow.weapon_type}) - damage: {Thunder_Bow.damage} enchanted: {is_enchanted}")
...
Elixir_of_Life = potion("Elixir of Life", "heal", 50)
Mana_Surge = potion("Mana Surge", "mana", 30)
Berserk_Brew = potion("Berserk Brew", "attack", 20)

Elixir_of_Life.quantity = 3
Mana_Surge.quantity = 5

print(f"{Elixir_of_Life.name} ({Elixir_of_Life.quantity}) - effect: {Elixir_of_Life.effect}) potency: {Elixir_of_Life.potency}")
print(f"{Mana_Surge.name} ({Mana_Surge.quantity}) - effect: {Mana_Surge.effect}) potency: {Mana_Surge.potency}")
print(f"{Berserk_Brew.name} ({Berserk_Brew.quantity}) - effect: {Berserk_Brew.effect}) potency: {Berserk_Brew.potency}")
